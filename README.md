...
Przykład po wywołaniu komendy 'docker ps'
```CONTAINER ID   IMAGE                    COMMAND                  CREATED       STATUS       PORTS                                         NAMES
c043e6c7fe34   projekt-aisd-backend     "sh -c 'dotnet test …"   4 hours ago   Up 4 hours   0.0.0.0:8001->8001/tcp, [::]:8001->8001/tcp   backend
fd2552916b0e   projekt-aisd-db          "docker-entrypoint.s…"   4 hours ago   Up 4 hours   0.0.0.0:5434->5432/tcp, [::]:5434->5432/tcp   krasnoludki_db
d4a7fe7227c8   projekt-aisd-generator   "tail -f /dev/null"      4 hours ago   Up 4 hours                                                 generator_skrypt
af22e8c2bd41   projekt-aisd-frontend    "/docker-entrypoint.…"   4 hours ago   Up 4 hours   0.0.0.0:3000->80/tcp, [::]:3000->80/tcp       frontend
```
...
Aby wyłączyć testy, trzeba zamienić linijkę command w kontenerze backend
```
command: sh -c "dotnet test krasnoludki.Tests/krasnoludki.Tests.csproj && dotnet run --project krasnoludki"
```
na poniższą: 
```
command: sh -c "dotnet run --project krasnoludki"
```
...
Drugą opcją na wygenerowanie danych jest użycie frontendu. w tym celu należy stworzyć nowy zestaw danych, a następnie kliknąć przycisk "Generuj"
